...
// ── Resolve detail field value by key ────────────────────────────────────────
function resolveFieldValue(key, person) {
  switch (key) {
    case "name":        return person.name || "—";
    case "aliases":     return normalizeAliases(person.aliases).join(" · ") || "—";
    case "dateOfBirth": return normalizeDOB(person.dateOfBirth).join(", ") || "—";
    case "nationality": return normalizeNationality(person.nationality).join(", ") || "—";
    case "program":     return person.program || "—";
    case "remarks":     return person.remarks || "—";
    default:            return "—";
  }
}
...
const SanctionsSearch = () => {
  ...
  const { lang } = useLang();
  const t = staticContent.search[lang];
      ...
      <div style={{ fontFamily:"'IBM Plex Sans',sans-serif", animation:"fadeUp .5s ease" }}
        ...
        <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:14,
          ...
          <div className="ss-search-row">
            <input className="ss-input" type="text" placeholder={t.searchPlaceholder}
              value={query} onChange={e => setQuery(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleSearch()}
              style={{ flex:1, padding:"12px 14px", background:C.s2,
            ...
            <button className="ss-btn ss-search-btn" onClick={handleSearch} disabled={loading}
              style={{ background: loading ? C.s2 : `linear-gradient(135deg,${C.cyan},${C.purple})`,
                color: loading ? C.text2 : C.bg, padding:"12px 24px", border:"none",
        ...
        {loading && (
          <div style={{ textAlign:"center", padding:"50px 0" }}>
            ...
            <p style={{ marginTop:14, color:C.text2, fontSize:14 }}>{t.loadingText}</p>
          </div>
        )}
        ...
        {!loading && hasSearched && results.length === 0 && (
          <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:14,
            ...
            <h4 style={{ color:C.text, margin:"0 0 8px", fontSize:17, fontWeight:600 }}>
              {t.noResultsTitle}
            </h4>
            <p style={{ color:C.text2, margin:0, fontSize:13 }}>{t.noResultsSub}</p>
          </div>
        )}
              ...
              <div className="ss-card-padding" style={{ padding:"18px 22px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <h4 className="ss-result-name"
                      style={{ margin:"0 0 5px", fontSize:17, fontWeight:700, color:C.text,
                ...
                {/* Score cards — labels from t.scoreLabels */}
                <div className="ss-scores">
                  {t.scoreLabels.map(({ key, label }, idx) => {
                    const colors = [C.cyan, C.purple, C.green];
                    return (
                      <div key={key} style={{ background:C.s2,
                        border:`1px solid ${C.border}`, borderRadius:9, padding:"9px 12px" }}>
                        <div style={{ fontSize:10, color:C.text2, marginBottom:3,
                          textTransform:"uppercase", letterSpacing:"0.4px" }}>{label}</div>
                        <div style={{ fontSize:16, fontWeight:700, color:colors[idx],
                          fontFamily:"'JetBrains Mono',monospace" }}>
                          {(item[key] ?? 0).toFixed(1)}%
                        </div>
                      </div>
                    );
                  })}
        ...
        {/* ── Details Modal ── */}
        {selectedPerson && (
          <div onClick={() => setSelectedPerson(null)}
            style={{ position:"fixed", inset:0, background:"rgba(6,9,18,0.85)",
              display:"flex", alignItems:"center", justifyContent:"center",
              zIndex:1000, backdropFilter:"blur(6px)", padding:"16px" }}>
              ...
              <div style={{ padding:"20px 22px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <span style={{
                      background:`${getSourceColor(selectedPerson.source)}22`,
                ...
                {/* Detail rows — driven by t.detailFields */}
                {t.detailFields.map(({ key, label }) => (
                  <div key={key} className="ss-detail-row ss-detail-grid"
                    ...
                    <div style={{ fontSize:13, color:C.text, lineHeight:1.5,
                      fontWeight: resolveFieldValue(key, selectedPerson) === "—" ? 400 : 500 }}>
                      {resolveFieldValue(key, selectedPerson)}
                    </div>
                  </div>
                ))}
