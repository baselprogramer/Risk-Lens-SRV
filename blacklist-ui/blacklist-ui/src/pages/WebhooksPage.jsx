...
export default function WebhooksPage() {
  ...
  const t = staticContent.webhooksContent[lang]; 
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t.webhookCard.deleteConfirm)) return;
    try {
  ...
  const renderWebhooks = () => {
    if (loading) return <div style={{ textAlign: "center", padding: 60, color: "#4a6a8a" }}><RefreshCw size={28} style={{ animation: "spin 1s linear infinite" }} /></div>;
    
    if (webhooks.length === 0) {
      return (
        <div style={{ ...S.card, textAlign: "center", padding: 60 }}>
          <Webhook size={40} color="#1a2d4a" style={{ marginBottom: 12 }} />
          <div style={{ color: "#4a6a8a", fontSize: "0.9rem" }}>{t.emptyState.webhooks}</div>
        </div>
      );
    }
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <span style={S.badge(wh.active)}>
                ...
                {wh.active ? t.webhookCard.active : t.webhookCard.inactive}
              </span>
              {wh.failureCount > 0 && (
                <span style={{ ...S.badge(false), background: "rgba(245,158,11,0.1)", color: "#f59e0b", borderColor: "rgba(245,158,11,0.2)" }}>
                  {wh.failureCount} {t.webhookCard.failures}
                </span>
              )}
            </div>
            <div style={{ fontSize: "0.92rem", fontWeight: 600, color: "#e2e8f0", marginBottom: 6, wordBreak: "break-all" }}>{wh.url}</div>
            ...
            {wh.lastTriggeredAt && (
              <div style={{ fontSize: "0.75rem", color: "#3a5a7a" }}>
                {t.webhookCard.lastTriggered} {new Date(wh.lastTriggeredAt).toLocaleString()}
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            <button style={{ ...S.btn, background: "rgba(0,212,255,0.08)", color: "#00d4ff", border: "1px solid rgba(0,212,255,0.2)", padding: "7px 14px" }}
              ...
              <Activity size={14} /> {t.webhookCard.logs}
            </button>
            <button style={{ ...S.btn, ...S.btnDanger, padding: "7px 14px" }} onClick={() => handleDelete(wh.id)}>
              <Trash2 size={14} />
            </button>
          </div>
        </div>
    ...
    if (deliveries.length === 0) {
      return (
        <div style={{ ...S.card, textAlign: "center", padding: 60 }}>
          ...
          <div style={{ color: "#4a6a8a" }}>{t.emptyState.deliveries}</div>
        </div>
      );
    }
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={S.badge(d.success)}>
              ...
              {d.success ? t.deliveryCard.success : t.deliveryCard.failed}
            </span>
            <span style={{ padding: "2px 10px", borderRadius: 20, background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.2)", color: "#8b5cf6", fontSize: "0.72rem", fontWeight: 700 }}>{d.event}</span>
            <span style={{ fontSize: "0.75rem", color: "#4a6a8a" }}>HTTP {d.statusCode} · {t.deliveryCard.attempt} {d.attempt}</span>
          </div>
          <div style={{ fontSize: "0.75rem", color: "#3a5a7a" }}>{new Date(d.createdAt).toLocaleString()}</div>
        </div>
        {d.responseBody && (
      ...
      <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", color: "#e2e8f0" }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div style={S.header}>
          <div style={S.title}>
            ...
            <span style={{ background: "linear-gradient(135deg,#e2e8f0,#00d4ff)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{t.title}</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid #1a2d4a" }}>
          <button style={S.tab(activeTab === "webhooks")} onClick={() => setActiveTab("webhooks")}>{t.tabs.webhooks} ({webhooks.length})</button>
          {selectedId && <button style={S.tab(activeTab === "deliveries")} onClick={() => setActiveTab("deliveries")}>{t.tabs.deliveries}</button>}
        </div>

        {showForm && (
          <div style={{ ...S.card, border: "1px solid rgba(0,212,255,0.2)", marginBottom: 24 }}>
            <div style={{ fontSize: "1rem", fontWeight: 700, marginBottom: 18, color: "#00d4ff" }}>{t.form.title}</div>
            <div style={{ marginBottom: 16 }}>
              <label style={S.label}>{t.form.urlLabel}</label>
              <input style={S.input} placeholder={t.form.urlPlaceholder} value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={S.label}>{t.form.eventTitle} *</label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {/* هنا استخدمت المصفوفة العامة التي عرفناها سابقاً للأحداث */}
                {t.events.map(ev => (
                  <label key={ev.key} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", padding: "7px 14px", borderRadius: 8, border: `1px solid ${form.events.includes(ev.key) ? "rgba(0,212,255,0.4)" : "#1a2d4a"}`, background: form.events.includes(ev.key) ? "rgba(0,212,255,0.08)" : "transparent", fontSize: "0.82rem", color: form.events.includes(ev.key) ? "#00d4ff" : "#4a6a8a", transition: "all 0.2s" }}>
                    <input type="checkbox" checked={form.events.includes(ev.key)} onChange={() => toggleEvent(ev.key)} style={{ accentColor: "#00d4ff" }} />
                    {ev.label}
                  </label>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={S.label}>{t.form.secretLabel}</label>
              <input style={S.input} placeholder={t.form.secretPlaceholder} value={form.secret} onChange={e => setForm(f => ({ ...f, secret: e.target.value }))} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button style={{ ...S.btn, ...S.btnPrimary }} onClick={handleCreate}><Plus size={15} /> {t.form.createBtn}</button>
              <button style={{ ...S.btn, background: "rgba(255,255,255,0.05)", color: "#4a6a8a" }} onClick={() => setShowForm(false)}>{t.form.cancelBtn}</button>
            </div>
          </div>
        )}
