      ...
      if (isPep) {
        try {
          ...
          setDetails(pepResult ? {
            ...
            notes: pepResult.notes || match.notes || t.pepFullTitle,
          ...
          setDetails({ name: match.matchedName, notes: match.notes || t.pepFullTitle, wikidataId: match.wikidataId || null });
        }
        setLoading(false);
        return;
      }
        ...
        <div style={{padding:"18px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:15,fontWeight:700,color:"#e2e8f0"}}>{isPep?t.pepDetailsTitle:t.entityDetailsTitle}</span>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            <div style={{background:"#111c2e",borderRadius:9,padding:"10px 12px",border:"1px solid #1a2d4a"}}>
              <div style={{fontSize:"0.62rem",color:"#3a5a7a",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:3}}>{t.matchScoreLabel}</div>
            ...
            <div style={{background:"#111c2e",borderRadius:9,padding:"10px 12px",border:"1px solid #1a2d4a"}}>
              <div style={{fontSize:"0.62rem",color:"#3a5a7a",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:3}}>{t.partyLabel}</div>
          ...
          {!loading && isPep && details && (
            <div style={{background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.3)",borderRadius:10,padding:"12px 14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:10}}>
                ...
                <span style={{fontSize:12,fontWeight:700,color:"#a78bfa"}}>{t.pepFullTitle}</span>
              </div>
              {renderRow(t.detailsFields.fullName,    details.name || match.matchedName)}
              {renderRow(t.detailsFields.description, details.notes || "—")}
              {details.wikidataId && renderRow(t.detailsFields.wikidataId,
          ...
          {!loading && !details && (
            <div style={{textAlign:"center",padding:"20px 0",color:"#7a8fa8",fontSize:13}}>{t.noDetailsAvailable}</div>
          )}
...
// ── Decision Modal ────────────────────────────────────────────────
function DecisionModal({ screeningId, onClose, onSaved, t }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onSaved(t.decisions.find(d=>d.value===decision));
      onClose();
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",
      ...
      <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:16,
        ...
        <div style={{fontSize:15,fontWeight:700,color:"#e2e8f0",marginBottom:4,display:"flex",alignItems:"center",gap:8}}>
          <Scale size={15} color="#00d4ff"/> {t.decisionModalTitle}
        </div>
        <div style={{fontSize:12,color:"#7a8fa8",marginBottom:14}}>{t.transferRefLabel}{screeningId}</div>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:"9px",background:"#111c2e",
            border:"1px solid #1a2d4a",color:"#7a8fa8",borderRadius:9,cursor:"pointer",fontSize:13}}>{t.decisionCancel}</button>
          <button onClick={handleSave} disabled={saving||!decision} style={{
            ...
            <Scale size={13}/>{saving?t.decisionSaving:t.decisionSave}
          </button>
        </div>
      </div>
    </div>
...
// ── KYC Section ───────────────────────────────────────────────────
function KycSection({ form, setForm, party, t }) {
  ...
  return (
    <div style={{marginTop:4}}>
      <button onClick={()=>setOpen(v=>!v)} style={{
        ...
        {open ? t.kycHide : t.kycAdd} {t.kycLabel}
        {hasData && !open && (
          <span style={{background:"rgba(0,212,255,0.15)",color:"#00d4ff",
            border:"1px solid rgba(0,212,255,0.3)",padding:"1px 6px",borderRadius:4,fontSize:10}}>{t.kycActive}</span>
        )}
      </button>
      {open && (
        <div style={{marginTop:8,display:"flex",flexDirection:"column",gap:8,animation:"fadeUp .15s ease"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div>
              <label style={{fontSize:"0.65rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:4}}>{t.nationalityLabel}</label>
            ...
            <div>
              <label style={{fontSize:"0.65rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:4}}>{t.dobLabel}</label>
              <input className="ts-inp" type="date" value={form[dobKey]||""} onChange={e=>setForm(p=>({...p,[dobKey]:e.target.value}))}/>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div>
              <label style={{fontSize:"0.65rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:4}}>{t.idTypeLabel}</label>
              <select className="ts-inp" value={form[idTKey]||""} onChange={e=>setForm(p=>({...p,[idTKey]:e.target.value}))}>
                <option value="">{t.selectPlaceholder}</option>
                {ID_TYPES.map(tp=><option key={tp.value} value={tp.value}>{tp.label}</option>)}
              </select>
            </div>
            <div>
              <label style={{fontSize:"0.65rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:4}}>{t.idNumberLabel}</label>
...
export default function TransferScreeningPage() {
  ...
  const handleScreen = async () => {
    if(!form.senderName.trim()||!form.receiverName.trim()){setError(t.errRequiredNames);return;}
    ...
    try {
      ...
      if(!text)throw new Error(t.errEmptyResponse);
      const data=JSON.parse(text);
      if(!res.ok)throw new Error(data.message||`${t.errGeneric} ${res.status}`);
  ...
  const openDetail = async (id) => {
    try {
      ...
      if(r.ok){
        ...
        if(d.ok){const dec=await d.json();if(dec)setModalDecision(t.decisions.find(x=>x.value===dec.decision));}
      }
    } catch{}
  };
  ...
  const ActionIcon = ({ action, size=28 }) => {
    const color = t.actionCFG[action]?.color || "#7a8fa8";
  ...
  return (
    <>
      ...
      <Layout>
        <div style={{maxWidth:1200,margin:"0 auto",animation:"fadeUp .4s ease"}} dir={lang === "ar" ? "rtl" : "ltr"}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7,background:"rgba(16,185,129,.08)",
              ...
              <span style={{fontSize:"0.7rem",fontWeight:700,color:"#10b981",fontFamily:"'JetBrains Mono',monospace"}}>{t.live}</span>
            </div>
          </div>
          ...
          {stats && (
            <div className="ts-stats-grid">
              {t.statsLabels.map(s=>(
                <div key={s.key} style={{background:"#0d1321",border:`1px solid ${s.color}22`,
          ...
          <div style={{display:"flex",gap:4,marginBottom:18,background:"#0d1321",
            ...
            {t.tabs.map(tb=>(
              <button key={tb.id} className="ts-tab" onClick={()=>setTab(tb.id)}
                style={{padding:"8px 18px",borderRadius:9,fontSize:"0.84rem",fontWeight:600,
                  display:"flex",alignItems:"center",gap:6,
                  color:tab===tb.id?"#060912":"#7a8fa8",
                  background:tab===tb.id?"linear-gradient(135deg,#00d4ff,#8b5cf6)":"transparent"}}>
            ...
            <div className="ts-screen-grid">
              <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:14,overflow:"hidden"}}>
                <div style={{padding:"12px 18px",borderBottom:"1px solid #1a2d4a",
                  ...
                  <span style={{fontSize:"0.75rem",fontWeight:700,color:"#7a8fa8",textTransform:"uppercase",letterSpacing:"0.6px"}}>{t.transferDetailsTitle}</span>
                </div>
                <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
                  {[
                    {key:"senderName",   label:t.senderNameLabel,   placeholder:t.fullNamePlaceholder,dir:"ltr"},
                    {key:"senderNameAr", label:t.senderNameArLabel, placeholder:t.fullNameArPlaceholder, dir:"rtl"},
                  ].map(f=>(
                    <div key={f.key}>
                  ...
                  <KycSection form={form} setForm={setForm} party="sender" t={t}/>
                  <div style={{height:1,background:"#1a2d4a"}}/>
                  {[
                    {key:"receiverName",   label:t.receiverNameLabel,   placeholder:t.fullNamePlaceholder,dir:"ltr"},
                    {key:"receiverNameAr", label:t.receiverNameArLabel, placeholder:t.fullNameArPlaceholder, dir:"rtl"},
                  ].map(f=>(
                    <div key={f.key}>
                      <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{f.label}</label>
                      <input className="ts-inp" value={form[f.key]} dir={f.dir}
                  ...
                  <KycSection form={form} setForm={setForm} party="receiver" t={t}/>
                  <div style={{height:1,background:"#1a2d4a"}}/>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10}}>
                    <div>
                      <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.amountLabel}</label>
                      <input className="ts-inp" type="number" value={form.amount}
                        onChange={e=>setForm(p=>({...p,amount:e.target.value}))} placeholder="0.00"/>
                    </div>
                    <div>
                      <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.ccyLabel}</label>
                  ...
                  <div>
                    <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.countryLabel}</label>
                  ...
                  <button onClick={()=>setShowExtra(v=>!v)} style={{
                    ...
                    {showExtra?t.extraDetailsHide:t.extraDetailsAdd} {t.extraDetailsLabel}
                  ...
                  {showExtra && (
                    <div style={{display:"flex",flexDirection:"column",gap:8,animation:"fadeUp .15s ease"}}>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                        <div>
                          <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.cityLabel}</label>
                          <input className="ts-inp" value={form.city} onChange={e=>setForm(p=>({...p,city:e.target.value}))} placeholder={t.cityPlaceholder}/>
                        </div>
                        <div>
                          <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.amountUsdLabel}</label>
                          <input className="ts-inp" type="number" value={form.amountInUsd} onChange={e=>setForm(p=>({...p,amountInUsd:e.target.value}))} placeholder={t.amountUsdPlaceholder}/>
                        </div>
                      </div>
                      <div>
                        <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.transferPurposeLabel}</label>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                        <div>
                          <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.agentNameLabel}</label>
                          <input className="ts-inp" value={form.agentName} onChange={e=>setForm(p=>({...p,agentName:e.target.value}))} placeholder={t.agentNamePlaceholder}/>
                        </div>
                        <div>
                          <label style={{fontSize:"0.7rem",fontWeight:700,color:"#3a5a7a",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5}}>{t.externalRefLabel}</label>
                          <input className="ts-inp" value={form.externalReference} onChange={e=>setForm(p=>({...p,externalReference:e.target.value}))} placeholder={t.externalRefPlaceholder}/>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <button onClick={handleScreen} disabled={loading}
                    ...
                    {loading
                      ? <><div style={{width:16,height:16,border:"2px solid rgba(0,212,255,.3)",borderTop:"2px solid #00d4ff",borderRadius:"50%",animation:"spin .8s linear infinite"}}/> {t.screeningBtn}</>
                      : <><Shield size={16}/> {t.screenTransferBtn}</>
                    }
                  </button>
                </div>
              </div>
              ...
              <div>
                {!result&&!loading&&(
                  <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:14,
                    ...
                    <div style={{fontSize:"0.88rem",fontWeight:600}}>{t.awaitingTitle}</div>
                    <div style={{fontSize:"0.73rem",textAlign:"center",maxWidth:200}}>{t.awaitingSub}</div>
                  </div>
                )}
                {loading&&(
                  <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:14,
                    ...
                    <div style={{fontSize:"0.82rem",color:"#7a8fa8"}}>{t.screeningProgress}</div>
                  </div>
                )}
                {result&&!loading&&(()=>{
                  const cfg=t.actionCFG[result.action]||t.actionCFG.REVIEW;
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:12}}>
                      <div style={{background:cfg.bg,border:`1px solid ${cfg.border}`,borderRadius:14,padding:"18px 20px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14,flexWrap:"wrap"}}>
                          ...
                          <div style={{textAlign:"right"}}>
                            <div style={{fontSize:"0.62rem",color:"#3a5a7a",fontFamily:"'JetBrains Mono',monospace"}}>{t.refLabel}</div>
                        ...
                        <div style={{paddingTop:12,borderTop:`1px solid ${cfg.border}`,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                          ...
                          {isAdmin()&&(
                            <button className="dec-btn" onClick={()=>setShowDecision(true)} style={{
                              ...
                              display:"flex",alignItems:"center",gap:6}}>
                              <Scale size={13}/> {t.recordDecisionBtn}
                            </button>
                          )}
                        </div>
                      </div>
                      <div className="ts-result-grid">
                        <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:11,padding:"12px 16px"}}>
                          <div style={{fontSize:"0.63rem",color:"#3a5a7a",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4}}>{t.riskLevelLabel}</div>
                          <div style={{fontSize:"1rem",fontWeight:700,color:riskCfg.color,fontFamily:"'JetBrains Mono',monospace"}}>{result.riskLevel}</div>
                        </div>
                        <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:11,padding:"12px 16px"}}>
                          <div style={{fontSize:"0.63rem",color:"#3a5a7a",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4}}>{t.riskPointsLabel}</div>
                      ...
                      {result.matches?.length>0&&(
                        <div style={{background:"#0d1321",border:"1px solid rgba(239,68,68,.2)",borderRadius:11,overflow:"hidden"}}>
                          <div style={{padding:"10px 14px",borderBottom:"1px solid rgba(239,68,68,.15)",fontSize:"0.72rem",fontWeight:700,color:"#ef4444",textTransform:"uppercase",letterSpacing:"0.5px",display:"flex",alignItems:"center",gap:6}}>
                            <AlertTriangle size={13}/> {result.matches.length} {result.matches.length>1?t.matchPlural:t.matchSingular} {t.clickToView}
                          </div>
                      ...
                      <button onClick={resetForm}
                        ...
                        <ArrowRight size={13} style={{transform: lang === "ar" ? "none" : "rotate(180deg)"}}/> {t.newScreeningBtn}
                      </button>
                    </div>
                  );
                })()}
            ...
            <div style={{background:"#0d1321",border:"1px solid #1a2d4a",borderRadius:14,overflow:"hidden"}}>
              <div style={{padding:"12px 18px",borderBottom:"1px solid #1a2d4a",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  ...
                  <span style={{fontSize:"0.75rem",fontWeight:700,color:"#7a8fa8",textTransform:"uppercase",letterSpacing:"0.6px"}}>{t.historyTitle}</span>
                </div>
                <span style={{fontSize:"0.7rem",color:"#3a5a7a",fontFamily:"'JetBrains Mono',monospace"}}>{t.pageLabel} {page+1}/{totalPages}</span>
              </div>
              <div className="ts-history-wrap">
                <table className="ts-history-table">
                  <thead>
                    <tr style={{background:"#111c2e"}}>
                      {t.historyHeaders.map((h,idx)=>(
                        <th key={idx} style={{padding:"9px 14px",textAlign:lang==="ar"?"right":"left",fontSize:"0.62rem",fontWeight:700,color:"#3a5a7a",letterSpacing:"0.7px",borderBottom:"1px solid #1a2d4a",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {!histLoading&&history.map(r=>{
                      const cfg=t.actionCFG[r.action]||t.actionCFG.REVIEW;
                    ...
                    {!histLoading&&history.length===0&&(
                      <tr><td colSpan={9} style={{padding:"40px",textAlign:"center"}}>
                        ...
                        <div style={{fontSize:"0.88rem",color:"#4a6a8a"}}>{t.noScreenings}</div>
                      </td></tr>
                    )}
                  </tbody>
                </table>
                  ...
                  return (
                    <div key={r.id} style={{padding:"12px 14px",borderBottom:"1px solid #111c2e",animation:`fadeUp .3s ease ${i*.04}s both`}}>
                      ...
                      <button onClick={()=>openDetail(r.id)}
                        ...
                        <FileText size={12}/> {t.viewDetailsBtn}
                      </button>
                    </div>
                  );
                })}
                {!histLoading&&history.length===0&&(
                  <div style={{padding:"40px 20px",textAlign:"center",color:"#4a6a8a"}}>{t.noScreenings}</div>
                )}
              </div>
              {totalPages>1&&(
                <div style={{padding:"12px 18px",borderTop:"1px solid #1a2d4a",display:"flex",justifyContent:"center",gap:8}}>
                  <button onClick={()=>setPage(p=>Math.max(0,p-1))} disabled={page===0}
                    style={{padding:"5px 12px",borderRadius:8,border:"1px solid #1a2d4a",background:"#111c2e",color:page===0?"#3a5a7a":"#94a3b8",cursor:page===0?"not-allowed":"pointer",fontSize:"0.78rem"}}>{t.prevBtn}</button>
                  ...
                  <button onClick={()=>setPage(p=>Math.min(totalPages-1,p+1))} disabled={page===totalPages-1}
                    style={{padding:"5px 12px",borderRadius:8,border:"1px solid #1a2d4a",background:"#111c2e",color:page===totalPages-1?"#3a5a7a":"#94a3b8",cursor:page===totalPages-1?"not-allowed":"pointer",fontSize:"0.78rem"}}>{t.nextBtn}</button>
                </div>
              )}
            </div>
          )}
          ...
          <div className="modal">
            <div style={{padding:"14px 18px",borderBottom:"1px solid #1a2d4a"}}>
              <div className="ts-modal-header">
                <div>
                  <div style={{fontSize:"0.84rem",fontWeight:700,color:"#e2e8f0"}}>{t.screeningDetailsTitle}</div>
            ...
            <div style={{padding:18,display:"flex",flexDirection:"column",gap:14}}>
              {(()=>{
                const cfg=t.actionCFG[selected.action]||t.actionCFG.REVIEW;
                return (
              ...
              <div className="ts-detail-grid">
                {[
                  {label:t.detailFieldLabels.sender,      value:selected.senderName},
                  {label:t.detailFieldLabels.receiver,    value:selected.receiverName},
                  {label:t.detailFieldLabels.amount,      value:selected.amount?`${selected.amount} ${selected.currency}`:"—"},
                  {label:t.detailFieldLabels.country,     value:selected.country||"—"},
                  {label:t.detailFieldLabels.city,        value:selected.city||"—"},
                  {label:t.detailFieldLabels.purpose,     value:selected.transferPurpose||"—"},
                  {label:t.detailFieldLabels.riskLevel,   value:selected.riskLevel, color:(RISK_CFG[selected.riskLevel]||{}).color},
                  {label:t.detailFieldLabels.riskPoints,  value:selected.riskPoints, mono:true},
                  {label:t.detailFieldLabels.speed,       value:`${selected.processingMs}ms`, mono:true},
                  {label:t.detailFieldLabels.screenedBy,  value:selected.createdBy||"—"},
                  ...(selected.operatorName?[{label:t.detailFieldLabels.operator,value:`${selected.operatorName}${selected.operatorId?` (${selected.operatorId})`:""  }`,color:"#00d4ff"}]:[]),
                  ...(selected.externalReference?[{label:t.detailFieldLabels.externalRef,value:selected.externalReference,mono:true,color:"#a78bfa"}]:[]),
                  ...(selected.agentName?[{label:t.detailFieldLabels.agent,value:selected.agentName}]:[]),
                  ...(selected.amountInUsd?[{label:t.detailFieldLabels.amountUsd,value:`$${selected.amountInUsd.toLocaleString()}`,mono:true}]:[]),
                  ...(selected.senderNationality?[{label:t.detailFieldLabels.senderNat,value:selected.senderNationality}]:[]),
                  ...(selected.receiverNationality?[{label:t.detailFieldLabels.receiverNat,value:selected.receiverNationality}]:[]),
                ].map(f=>(
              ...
              {selected.matches?.length>0&&(
                <div style={{background:"#111c2e",borderRadius:11,overflow:"hidden",border:"1px solid rgba(239,68,68,.2)"}}>
                  <div style={{padding:"9px 14px",borderBottom:"1px solid rgba(239,68,68,.15)",fontSize:"0.7rem",fontWeight:700,color:"#ef4444",textTransform:"uppercase",letterSpacing:"0.5px",display:"flex",alignItems:"center",gap:6}}>
                    <AlertTriangle size={12}/> {t.matchPlural} ({selected.matches.length}) {t.clickToView}
                  </div>
