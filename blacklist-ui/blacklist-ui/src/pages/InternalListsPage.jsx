...
import {
  getAllInternal, createInternal, updateInternal,
  deleteInternal, importInternalFile
} from "../services/internalListService";
...
const InternalListsPage = () => {
  ...
  const loadData = async () => {
    try { setLoading(true); setError(null); setRecords(await getAllInternal()); }
    catch (e) { setError("فشل تحميل القائمة الداخلية."); }
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (selected) {
        await updateInternal(selected.id, data);
      ...
      await loadData();
    } catch (e) { setError(e.message||"فشل"); showToast(e.message||"فشل","error"); }
    finally { setLoading(false); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await deleteInternal(confirmDelete.id);
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true); setError(null);
      const result = await importInternalFile(file);
      showToast(`✓ تم استيراد ${result.saved} سجل بنجاح`);
...
export default InternalListsPage;